...
(function($) {
    ...
    var event_handlers = {
        ...
        dialogClick : function(e) {
			...
			//menu.selectedIndex = $(e.target).index();
			

			//($('select'))[0].prop('selectedIndex', 5);
			//$('select').prop('selectedIndex', 5);
			//var menu = ($('body').find('select'))[0];
            ...
            if (clicked.is("li") && !clicked.hasClass("gentleselect-dummy")) {
                ...
                if ($this.data("root").attr("multiple")) {
					...
					$("select")[0].selectedIndex = $(e.target).index();
